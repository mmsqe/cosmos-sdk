	...
	for _, tc := range testCases {
		...
		for _, worker := range []int{1, 5, 10, 15, 20} {
			b.Run(tc.name+"-worker-"+strconv.Itoa(worker), func(b *testing.B) {
				...
				// Mimic the executor path used elsewhere by providing per-txn caches.
				incarnationCache := make([]atomic.Pointer[Cache], tc.block.Size())
...
func abaSameValueBlockWithValue(keys [][]byte, value []byte) *MockBlock {
	...
	for i := range keys {
		idx := i
		txs[i] = func(store MultiStore, cache Cache) error {
