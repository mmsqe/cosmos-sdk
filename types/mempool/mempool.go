...
// PooledTx is a mempool entry pairing the transaction with the gas wanted value
// reported by the ante handler at insert time. Mempool implementations store and
// return PooledTx so callers can access the authoritative gas value without
// re-running the ante handler.
...
// InsertOption carries optional metadata for a mempool Insert call, e.g. the
// gas wanted reported by the ante handler.
type InsertOption struct {
	GasWanted uint64
}

type Mempool interface {
	// Insert attempts to insert a tx into the app-side mempool together with
	// ante-handler-reported metadata (e.g. gas wanted), returning an error upon
	// failure.
