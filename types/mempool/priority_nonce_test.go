...
// gasTestTx wraps testTx and implements sdk.GasTx with a configurable declared
// gas value. Used to assert that the mempool stores the ante-handler-reported
// gas (InsertOption.GasWanted) rather than falling back to tx.GetGas().
type gasTestTx struct {
	testTx
	declaredGas uint64
}

func (g gasTestTx) GetGas() uint64 { return g.declaredGas }

// TestMempoolStoresAnteGasNotTxGas is the regression test for the bug fixed by
// this PR: when tx.GetGas() differs from the gas wanted reported by the ante
// handler, the mempool must store the ante value (via InsertOption.GasWanted)
// so that PrepareProposal selects txs based on the authoritative gas figure.
func TestMempoolStoresAnteGasNotTxGas(t *testing.T) {
	const (
		txDeclaredGas = uint64(1_000_000) // what tx.GetGas() reports (e.g. user-requested limit)
		anteGasWanted = uint64(50_000)    // what the ante handler actually decided
	)

	mp := mempool.DefaultPriorityMempool()
	accounts := simtypes.RandomAccounts(rand.New(rand.NewSource(0)), 1)
	tx := gasTestTx{
		testTx:      testTx{priority: 10, nonce: 0, address: accounts[0].Address},
		declaredGas: txDeclaredGas,
	}
	ctx := sdk.NewContext(nil, cmtproto.Header{}, false, log.NewNopLogger()).WithPriority(tx.priority)

	require.NoError(t, mp.Insert(ctx, tx, mempool.InsertOption{GasWanted: anteGasWanted}))

	var stored []mempool.PooledTx
	mp.SelectBy(ctx, nil, func(pt mempool.PooledTx) bool {
		stored = append(stored, pt)
		return true
	})
	require.Len(t, stored, 1)
	require.Equal(t, anteGasWanted, stored[0].GasWanted,
		"mempool must store the ante handler's gasWanted, not tx.GetGas()")
	require.NotEqual(t, txDeclaredGas, stored[0].GasWanted,
		"mempool must not silently fall back to tx.GetGas()")
}
