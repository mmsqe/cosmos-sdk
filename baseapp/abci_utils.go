...
func (h *DefaultProposalHandler) ProcessProposalHandler() sdk.ProcessProposalHandler {
	...
	return func(ctx sdk.Context, req *abci.RequestProcessProposal) (*abci.ResponseProcessProposal, error) {
		...
		for _, txBytes := range req.Txs {
			...
			if maxBlockGas > 0 {
				// Defense-in-depth: if the ante handler reports gasWanted == 0
				// but the tx itself declares a non-zero gas limit, fall back to
				// the declared value for block-gas accounting. This prevents a
				// buggy or permissive ante from letting a proposer slip a block
				// past MaxGas. The mempool entry's stored GasWanted (from
				// CheckTx's ante) remains the source of truth for tx ordering;
				// this fallback only affects the over-limit guard here.
				txGas := gasWanted
				if txGas == 0 {
					if gasTx, ok := tx.(sdk.GasTx); ok {
						txGas = gasTx.GetGas()
...
func (ts *defaultTxSelector) SelectTxForProposal(_ context.Context, maxTxBytes, maxBlockGas uint64, memTx sdk.Tx, txBz []byte, gasWanted uint64) bool {
	...
	if (txSize + ts.totalTxBytes) <= maxTxBytes {
		...
		// not been met.
		if maxBlockGas > 0 {
			// Fall back to the tx's declared gas if the ante reported 0; this
			// mirrors the ProcessProposal guard so a proposer can't pack a
			// block its own ProcessProposal would then reject.
			txGas := gasWanted
			if txGas == 0 {
				if gasTx, ok := memTx.(sdk.GasTx); ok {
					txGas = gasTx.GetGas()
				}
			}
