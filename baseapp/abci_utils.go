	...
	return func(ctx sdk.Context, req *abci.PrepareProposalRequest) (*abci.PrepareProposalResponse, error) {
		...
		if h.mempool == nil || isNoOp {
			for i, tx := range decodedTxs {
				...
				stop := h.txSelector.SelectTxForProposal(ctx, uint64(req.MaxTxBytes), maxBlockGas, tx, req.Txs[i], txGasLimit)
				if stop {
					break
				}
			}
		...
		h.mempool.SelectBy(ctx, decodedTxs, func(memTx mempool.Tx) bool {
